...
// FIXME clean up warnings
public abstract class RangeSlider extends JSlider implements ValueRefresher {
  ...
  public void setUpperValue(final int value) {
    ...
    final int newExtent = Math.min(Math.max(0, value - lowerValue), getMaximum()
        - lowerValue);
...
class RangeSliderUI extends BasicSliderUI {
    ...
    if(slider.getSnapToTicks()) {
      ...
      if(tickSpacing != 0) {
        ...
        if((upperValue - slider.getMinimum()) % tickSpacing != 0) {
          final float temp = (float) (upperValue - slider.getMinimum()) / tickSpacing;
  ...
  public void scrollByBlock(final int direction) {
    synchronized(slider) {
      ...
      final int delta = blockIncrement
